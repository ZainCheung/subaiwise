...
describe('checked-in dataset channel coverage', () => {
  ...
  it('keeps Devin Max as a third-party OpenAI access channel after sync', () => {
    const dataset = SubAIWiseDatasetSchema.parse(
      JSON.parse(readFileSync(resolve(process.cwd(), 'data/dataset.json'), 'utf8')),
    )
    const entry = dataset.entries.find((item) => item.id === 'devin_max::gpt-6-astra')
    expect(entry).toMatchObject({
      provider: 'OpenAI',
      channel: 'Devin',
      plan: { id: 'devin_max', name: 'Devin Max' },
      model: { id: 'gpt-6-astra' },
    })
    expect(isThirdParty(entry!.provider, entry!.channel)).toBe(true)
  })
})
